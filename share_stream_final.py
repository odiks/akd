...
# ==============================================================================
# SECTION 1: FONCTIONS D'INTERACTION AVEC L'API GRAYLOG
# ==============================================================================

def _make_api_request(method: str, endpoint: str, payload: dict = None) -> dict | None:
    """Fonction d'aide générique pour les requêtes à l'API Graylog."""
    if not API_TOKEN:
    ...
    headers = {'Accept': 'application/json', 'Content-Type': 'application/json', 'X-Requested-By': 'interactive-stream-sharer-v2'}

    try:
        response = requests.request(
            method, api_url, headers=headers, auth=(API_TOKEN, 'token'), json=payload
        )
...
def get_all_streams() -> list[dict] | None:
    ...
    if response_data and 'streams' in response_data:
        streams_list = [
            {"stream_name": s.get('title'), "stream_id": s.get('id'), "creator_username": s.get('creator_user_id')}
...
def get_user_map() -> dict[str, str] | None:
    """
    Récupère tous les utilisateurs et retourne un mapping {username: user_id}.
    C'est la logique intégrée de find_user_id.py.
    """
    print("⏳ Récupération des utilisateurs et création du mapping username -> user_id...")
    ...
    if response_data and 'users' in response_data:
        ...
        print(f"✅ Mapping créé pour {len(user_map)} utilisateurs.")
        return user_map
    return None
...
def add_users_to_stream(stream_id: str, users_to_add: list[dict]) -> bool:
    ...
    for user in users_to_add:
        ...
        print(f"   + Ajout/Mise à jour de l'ID '{user['id']}' avec le rôle '{user['role']}'.")
...
if __name__ == "__main__":
    ...
    # ÉTAPE 1: RÉCUPÉRER TOUTES LES DONNÉES ET CRÉER LE MAPPING
    all_streams = get_all_streams()
    user_map = get_user_map() # Intégration de la logique de find_user_id.py
    if not all_streams or not user_map:
        print("\nImpossible de continuer sans les données des streams et le mapping des utilisateurs.", file=sys.stderr)
        sys.exit(1)
    ...
    if not selected_stream: sys.exit(1)
        
    stream_id = selected_stream['stream_id']
    creator_username = selected_stream['creator_username']

    # ÉTAPE 2.1: TRADUIRE LE USERNAME DU CRÉATEUR EN VÉRITABLE USER_ID
    owner_id = user_map.get(creator_username)
    if not owner_id:
        print(f"❌ Erreur critique: Impossible de trouver l'ID pour le créateur '{creator_username}'.", file=sys.stderr)
        print("   Le stream est peut-être orphelin (utilisateur créateur supprimé). Opération annulée.", file=sys.stderr)
        sys.exit(1)
    
    print(f"\nStream sélectionné : '{selected_stream['stream_name']}'")
    print(f"  ID du Stream : {stream_id}")
    print(f"  Propriétaire (créateur) : {creator_username} (ID résolu : {owner_id})")
    ...
    users_to_add = []
    # Créer une liste d'utilisateurs sous forme de dictionnaires pour la sélection
    all_users_list = [{'username': u, 'user_id': i} for u, i in user_map.items()]
    
    while len(users_to_add) < num_to_add:
        print(f"\n--- Sélection de l'utilisateur {len(users_to_add) + 1}/{num_to_add} ---")
        
        # Exclure le propriétaire et les utilisateurs déjà sélectionnés
        selectable_users = [
            u for u in all_users_list 
            if u['user_id'] != owner_id and u['user_id'] not in [sel['id'] for sel in users_to_add]
        ]
    ...
    for u in users_to_add:
        username = next((item['username'] for item in all_users_list if item['user_id'] == u['id']), u['id'])
        print(f"  - {username} (ID: {u['id']})")
